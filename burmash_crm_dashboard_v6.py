# -*- coding: utf-8 -*-
"""
БУРМАШ · CRM Дэшборд (v6.0)
— Новая боковая панель управления: 🔄 Обновить, 🗑️ Очистить, 🕒 Время обновления, 📊 Статистика кэша.
— Подсказки (help=...), кнопки на всю ширину, авто‑перезапуск после очистки.
— Кэширование на диск (persist="disk"): активности 15 мин, сделки/история 30 мин, справочники 60 мин.
— Batch к Bitrix24: стадии (до 50 категорий/запрос), история стадий (до 50 сделок/запрос).
— Оптимизация загрузки с прогрессами «⏳ Загрузка сделок…», «📅 Загрузка активностей…» и т.п.

Требуется: Streamlit ≥ 1.33, requests, numpy, pandas, plotly (необязательно).
"""
...
# КОНСТАНТЫ КЭША/МЕТА
# =====================
TTL_ACTIVITIES = 15*60    # 15 минут
TTL_DEALS      = 30*60    # 30 минут
TTL_HISTORY    = 30*60    # 30 минут
TTL_DICTS      = 60*60    # 1 час (пользователи/отделы/категории)
META_PATH      = os.path.join(os.getcwd(), ".burmash_cache_meta.json")

# refresh token для принудительного обновления cache_data
...
# =====================
# КЭШИРУЕМЫЕ ФУНКЦИИ (persist="disk")
# =====================
...
# Batch‑вариант для стадий — до 50 категорий за запрос
@st.cache_data(ttl=TTL_DICTS, persist="disk")
def bx_get_stage_map_by_category_batch(category_ids, _buster=""):
    sort_map, name_map = {}, {}
    if not category_ids:
        return sort_map, name_map
    ids = [int(x) for x in category_ids if pd.notna(x)]
    for chunk_idx in range(0, len(ids), 50):
        chunk = ids[chunk_idx:chunk_idx+50]
        cmds = {f"c{i}": f"crm.dealcategory.stage.list?id={cid}" for i, cid in enumerate(chunk)}
        try:
    ...
    # Фоллбек на базовую таблицу стадий
    if not name_map:
        try:
...
@st.cache_data(ttl=TTL_DICTS, persist="disk")
def bx_get_departments(_buster=""):
    try:
...
def bx_get_users_full(_buster=""):
    ...
    for u in users:
        ...
        out[int(u["ID"])] = {
            "name": ((u.get("NAME","")+" "+u.get("LAST_NAME",""))).strip() or u.get("LOGIN",""),
...
def bx_get_activities(deal_ids, include_completed=True, _buster=""):
    ...
    for state in states:
        # группировка по 40 сделок за проход
        chunk_size = 40
        ids = list(map(int, deal_ids))
...
# Batch‑история стадий — до 50 сделок/запрос
@st.cache_data(ttl=TTL_HISTORY, persist="disk")
def bx_get_stage_history_lite_batch(deal_ids, max_deals=300, _buster=""):
    if not deal_ids: return {}
    hist = {}
    ids = list(map(int, deal_ids))[:max_deals]
    # сначала crm.stagehistory.deal.list
    for chunk_idx in range(0, len(ids), 50):
        chunk = ids[chunk_idx:chunk_idx+50]
        cmds = {f"d{i}": f"crm.stagehistory.deal.list?filter[OWNER_ID]={did}" for i, did in enumerate(chunk)}
        try:
            res = _bx_batch(cmds, halt=0)
        except Exception:
            res = {}
        for _, items in (res or {}).items():
            if not items: continue
            try:
                did = int((items[0] or {}).get("OWNER_ID") or 0)
            except Exception:
                did = None
            if did:
                hist[did] = items
    # фоллбек — crm.stagehistory.list
    remain = [i for i in ids if i not in hist]
    for chunk_idx in range(0, len(remain), 50):
        chunk = remain[chunk_idx:chunk_idx+50]
        cmds = {f"s{i}": f"crm.stagehistory.list?filter[OWNER_TYPE_ID]=2&filter[OWNER_ID]={did}" for i, did in enumerate(chunk)}
        try:
            res = _bx_batch(cmds, halt=0)
        except Exception:
            res = {}
...
def compute_health_scores(df, open_tasks_map, stuck_days=5):
    ...
    for _, r in df.iterrows():
        ...
        rows.append({
            ...
            "STAGE_SEMANTIC_ID": (r.get("STAGE_SEMANTIC_ID") or "").upper(),
            "days_in_work": d_work,
...
with st.sidebar:
    ...
    if col_logout.button("Выйти", key="logout_btn", help="Завершить сессию"):
        st.session_state[AUTH_KEY] = False
        st.rerun()

    st.markdown("---")
    st.subheader("🎛️ Управление")
    btn_refresh = st.button("🔄 Обновить", use_container_width=True,
                            help="Принудительно загрузить свежие данные из Битрикс24, игнорируя кэш.")
    btn_clear = st.button("🗑️ Очистить", use_container_width=True,
                          help="Полностью очистить кэш приложения (память и диск). Автоматически перезапустит приложение.")
    ...
    with st.expander("📊 Статистика кэша", expanded=False):
        st.markdown(
            ...
            **Последняя сводка:**
            ...
            - История стадий: {meta.get('stats',{}).get('hist_cnt','—')}
            - Категории/Стадии: {meta.get('stats',{}).get('cats_cnt','—')}
...
try:
    ...
    status.update(label="📇 Загрузка пользователей/категорий…", state="running")
    ...
    sort_map, name_map = bx_get_stage_map_by_category_batch(cat_ids, _buster=st.session_state["__refresh_token__"])
    ...
    SUCCESS_NAME_BY_CAT = {CAT_MAIN: "Успешно реализовано", CAT_PHYS: "Сделка успешна", CAT_LOW:  "Сделка успешна"}
    ...
    status.update(label="🕓 История стадий (опционально)…", state="running")
    history_info = {}
    if use_history:
        try:
            history_raw = bx_get_stage_history_lite_batch(df_all["ID"].astype(int).tolist(), max_deals=history_limit, _buster=st.session_state["__refresh_token__"])
    ...
    departments = bx_get_departments(_buster=st.session_state["__refresh_token__"])
    sales_depts = [d for d in departments if "продаж" in (d.get("NAME","").lower())]
    sales_dept_ids = {int(d["ID"]) for d in sales_depts}
    def _ss(k, v):
        if k not in st.session_state: st.session_state[k] = v
    _ss("flt_sales_only", True if sales_dept_ids else False)
    ...
    if selected_dept_ids:
        users_full_all = bx_get_users_full(_buster=st.session_state["__refresh_token__"])
    ...
    df_created = df_all[m_created].copy()
    df_closed  = df_all[m_closed].copy()
    df_mod     = df_all[m_modify].copy()

    meta["last_updated"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S") if st.session_state["__refresh_token__"] else (meta.get("last_updated") or datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    meta["stats"] = {
        "deals_cnt": int(len(df_all)),
        "acts_cnt": int(sum(len(v) for v in activities.values())),
        "hist_cnt": int(len(history_info) if history_info else 0),
        "cats_cnt": int(len(categories)),
        "users_cnt": int(len(users_full)),
    }
    _write_meta(meta)

    status.update(label="✅ Данные готовы", state="complete")
except Exception as e:
    status.update(label=f"Ошибка загрузки: {e}", state="error")
    st.stop()
...
TAB_TITLES = ["📊 Обзор", "⚠️ Проблемы", "👥 По менеджерам", "🗂 Градация", "⏱ Время на этапах", "🤖 AI-аналитика", "📅 План/факт"]
tab_over, tab_prob, tab_mgr, tab_grad, tab_time, tab_ai, tab_plan = st.tabs(TAB_TITLES)
...
with tab_over:
    ...
    target_cats = {"основная воронка продаж", "физ.лица", "не приоритетные сделки"}
    ...
    per_cat = []
    for cat in ["основная воронка продаж", "физ.лица", "не приоритетные сделки"]:
    ...
    if px:
        ...
        if not ts_rev_by_cat.empty:
            fig_r = px.line(ts_rev_by_cat, x="period", y="value", color="cat",
                            ...
                            color_discrete_map={"основная воронка продаж":"#111111", "физ.лица":"#ff7a00", "не приоритетные сделки":"#999999"})
    ...
    for cat, title in [("основная воронка продаж", "Основная воронка продаж"), ("физ.лица", "Физ.Лица"), ("не приоритетные сделки", "Не приоритетные сделки")]:
        ...
        with st.expander(f"Воронка: {title}"):
            if px and not stage.empty:
                fig_v = px.funnel(stage, y="stage_name", x="Количество", color_discrete_sequence=["#111111" if cat=="основная воронка продаж" else "#ff7a00"])
...
with tab_prob:
    ...
    a.metric("Без задач", problems["Без задач"]) ; b.metric("Без компании", problems["Без компании"]) ; c.metric("Без контакта", problems["Без контакта"]) ; d.metric("Застряли", problems["Застряли"]) ; e.metric("Проиграны", problems["Проиграны"]) 
...
with tab_mgr:
    ...
    succ = df_closed[(df_closed["is_success"]) & (df_closed["cat_norm"].isin({"основная воронка продаж","физ.лица","не приоритетные сделки"}))].copy()
    ...
    for cat, title in [("основная воронка продаж","Основная воронка продаж"), ("физ.лица","Физ.Лица"), ("не приоритетные сделки","Не приоритетные сделки")]:
        ...
        with right:
            if "fail_from_stage_hist" in df_all.columns and df_all["fail_from_stage_hist"].notna().any():
...
with tab_plan:
    ...
    succ_y = df_all[(df_all["is_success"]) & (df_all["cat_norm"].isin({"основная воронка продаж","физ.лица","не приоритетные сделки"}))].copy()
...
st.markdown("---")
st.caption("БУРМАШ · CRM Дэшборд v6.0 — batch + дисковый кэш + панель управления")
