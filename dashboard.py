# -*- coding: utf-8 -*-
"""
БУРМАШ · CRM Дэшборд v4.2 (fixed)
— Исправление StreamlitDuplicateElementId: добавлены уникальные key для всех plotly_chart
...
with st.spinner("Загружаю данные…"):
    if not BITRIX24_WEBHOOK:
        st.error("Задайте BITRIX24_WEBHOOK в Secrets"); st.stop()
...
if view_df.empty:
    st.warning("Нет данных по выбранным фильтрам."); st.stop()
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    "📊 Обзор", "⚠️ Проблемы", "👤 По менеджерам", "🎯 Градация сделок", "⏱️ Время на этапах", "🤖 AI-аналитика"
])
...
with tab1:
    ...
    if px:
        ...
        st.plotly_chart(fig1, use_container_width=True, key="chart_overview_hist")
    ...
    if px:
        ...
        st.plotly_chart(fig2, use_container_width=True, key="chart_top_stages")
    ...
    if px:
        ...
        st.plotly_chart(fig3, use_container_width=True, key="chart_revenue_pie")
...
with tab2:
    ...
    if px:
        ...
        st.plotly_chart(fig_prob, use_container_width=True, key="chart_problems_bar")

    with st.expander(f"❗ Сделки без задач ({len(no_tasks)})"):
        if not no_tasks.empty:
            st.dataframe(no_tasks[["ID сделки","Название","Менеджер","Сумма","Здоровье"]], use_container_width=True)
    with st.expander(f"🏢 Сделки без компании ({len(no_company)})"):
        if not no_company.empty:
            st.dataframe(no_company[["ID сделки","Название","Менеджер","Сумма","Здоровье"]], use_container_width=True)
    with st.expander(f"📇 Сделки без контакта ({len(no_contact)})"):
        if not no_contact.empty:
            st.dataframe(no_contact[["ID сделки","Название","Менеджер","Сумма","Здоровье"]], use_container_width=True)
    with st.expander(f"⏸️ Застрявшие сделки ({len(stuck)})"):
        if not stuck.empty:
            st.dataframe(stuck[["ID сделки","Название","Менеджер","Дней без активности","Здоровье"]], use_container_width=True)
    with st.expander(f"❌ Проигранные сделки ({len(lost)})"):
        if not lost.empty:
            st.dataframe(lost[["ID сделки","Название","Менеджер","Сумма","Название этапа"]], use_container_width=True)
...
with tab3:
    ...
    if px and not df_mgr.empty:
        ...
        st.plotly_chart(fig4, use_container_width=True, key="chart_mgr_revenue")

        fig5 = px.scatter(df_mgr, x="Сделок", y="Конверсия в WON, %",
                          size="Выручка, ₽", hover_data=["Менеджер"],
                          title="Сделки vs Конверсия")
        st.plotly_chart(fig5, use_container_width=True, key="chart_mgr_scatter")
    ...
    for idx, row in conv_data.iterrows():
        with st.expander(f"👤 {row['Менеджер']} | {row['Воронка']} ({row['Всего сделок']} сделок)"):
            ...
            if px and not stage_df.empty:
                ...
                st.plotly_chart(fig6, use_container_width=True, key=f"chart_conv_funnel_{idx}")
...
with tab4:
    ...
    drop  = view_df[view_df["Стоп-лист?"]].sort_values("Скор отказа", descending=True) if "descending" in dir(pd.DataFrame.sort_values) else view_df[view_df["Стоп-лист?"]].sort_values("Скор отказа", ascending=False)
    ...
    if px:
        ...
        st.plotly_chart(fig7, use_container_width=True, key="chart_gradation")
...
with tab5:
    ...
    if px and not stage_time.empty:
        ...
        st.plotly_chart(fig8, use_container_width=True, key="chart_stage_time")
