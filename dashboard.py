# -*- coding: utf-8 -*-
"""
БУРМАШ · CRM Дэшборд v4.2.1
— Фикс авторизации (форма + session_state, не сбрасывается при фильтрах)
— Уникальные key у всех plotly_chart (исправлен StreamlitDuplicateElementId)
— Остальной код идентичен v4.2
"""
...
# --- Надёжная авторизация (вместо прежнего check_password) ---
...
# ============ HEADER ============
st.markdown("# 🟧 БУРМАШ · CRM Дэшборд v4.2.1")
...
with tab1:
    ...
    if px:
        ...
        st.plotly_chart(fig1, use_container_width=True, key="ov_hist")
    ...
    if px:
        ...
        st.plotly_chart(fig2, use_container_width=True, key="ov_top_stages")
    ...
    if px:
        ...
        st.plotly_chart(fig3, use_container_width=True, key="ov_revenue_pie")
...
with tab2:
    ...
    if px:
        ...
        st.plotly_chart(fig_prob, use_container_width=True, key="prob_bar")
    ...
    for idx, row in conv_data.iterrows():  # добавили idx
        with st.expander(f"👤 {row['Менеджер']} | {row['Воронка']} ({row['Всего сделок']} сделок)"):
            ...
            if px and not stage_df.empty:
                ...
                st.plotly_chart(fig6, use_container_width=True, key=f"conv_funnel_{idx}")
...
with tab4:
    ...
    if px:
        ...
        st.plotly_chart(fig7, use_container_width=True, key="gradation_bar")
...
with tab5:
    ...
    if px and not stage_time.empty:
        ...
        st.plotly_chart(fig8, use_container_width=True, key="stage_time_bar")
...
with tab7:
    ...
    if px:
        ...
        st.plotly_chart(fig_plan, use_container_width=True, key="year_plan_area")
...
st.markdown("---")
st.caption("БУРМАШ · CRM Дэшборд v4.2.1 | Powered by Bitrix24 + Perplexity AI")
